<template>
  ...
  const props = defineProps({
    ...
    maxValue: {
      type: Number,
      default: 8000  // Default for RPM
    },
    type: {
      type: String,
      default: 'rpm',
      validator: (value) => ['rpm', 'speed'].includes(value)
    },
    decimals: {
      type: Number,
      default: 0
  ...
  const ticks = computed(() => {
    ...
    for (let i = 0; i <= numMajorTicks; i++) {
      ...
      // Determine tick color based on value
      let tickColor = "#374151"; // Default color
      
      if (props.type === 'rpm' && value >= props.maxValue * 0.75) {
        tickColor = "#ef4444"; // Red for high RPM range (above 75% of max)
      }
      ...
      if (i < numMajorTicks) {
        for (let j = 1; j < 5; j++) {
          ...
          if (props.type === 'rpm' && minorValue >= props.maxValue * 0.75) {
            minorTickColor = "#ef4444"; // Red for high RPM range
          }
  ...
  const statusBadgeClass = computed(() => {
    if (status.value === "High") return "badge-error";
    if (status.value === "Moderate") return "badge-warning";
